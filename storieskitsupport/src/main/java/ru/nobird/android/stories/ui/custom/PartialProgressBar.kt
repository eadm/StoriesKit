...
constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : View(context, attrs, defStyleAttr) {
    ...
    init {
        val typedArray = context.obtainStyledAttributes(attrs, R.styleable.PartialProgressBar)
        try {
            progressBackgroundColor = typedArray.getColor(R.styleable.PartialProgressBar_progressBackgroundColor, progressBackgroundColor)
            progressForegroundColor = typedArray.getColor(R.styleable.PartialProgressBar_progressForegroundColor, progressForegroundColor)

            gap = typedArray.getDimension(R.styleable.PartialProgressBar_gap, gap)
            radius = typedArray.getDimension(R.styleable.PartialProgressBar_radius, radius)
        } finally {
            typedArray.recycle()
        }
    }
